...
async function requireCreditsUserId(ctx: ActionCtx, token: string | undefined): Promise<Id<"users">> {
...
async function key(ctx: ActionCtx, name: string): Promise<string> {
    const value = await ctx.runQuery(internal.config.get, { name });
    if (!value) throw new Error(`${name} is not configured (set it in Admin → Integrations)`);
    return value;
}

export const caption = action({
    args: { token: v.optional(v.string()), prompt: v.string(), tone: v.optional(v.string()), platforms: v.optional(v.array(v.string())) },
    handler: async (ctx, args): Promise<AiResult> => {
        const userId = await requireCreditsUserId(ctx, args.token);
        const apiKey = await key(ctx, "OPENAI_API_KEY");
...
export const image = action({
    args: { token: v.optional(v.string()), prompt: v.string() },
    handler: async (ctx, args): Promise<AiResult> => {
        const userId = await requireCreditsUserId(ctx, args.token);
        const apiKey = await key(ctx, "OPENAI_API_KEY");
        ...
        const storageId = await ctx.storage.store(new Blob([Buffer.from(b64, "base64")], { type: "image/png" }));
...
export const voice = action({
    args: { token: v.optional(v.string()), text: v.string(), voiceId: v.optional(v.string()) },
    handler: async (ctx, args): Promise<AiResult> => {
        const userId = await requireCreditsUserId(ctx, args.token);
        const apiKey = await key(ctx, "ELEVENLABS_API_KEY");
